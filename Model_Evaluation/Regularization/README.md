...
Regularization adds a penalty term to the loss function to discourage complex models with large coefficients. It prevents overfitting by keeping the model simpler.

**Simple Analogy**: Like a teacher penalizing complicated answers: the model learns to explain data simply rather than memorizing every detail.

## 2. Why do we need it?

Without regularization, models with many features often overfit, performing well on training data but poorly on new data.
